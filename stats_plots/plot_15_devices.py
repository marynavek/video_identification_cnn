...
path_to_csv = '/Users/marynavek/Projects/video_identification_cnn/ensemble_results/15_devices_2_patches_only/statistics/most_common_label_by_video'

previous_files = get_prediction_files(path_to_csv)

output_folder = "/Users/marynavek/Projects/video_identification_cnn/ensemble_results/15_devices_2_patches_only/statistics"
            ...
            with open(path, newline='') as csvfile:
                ...
                for device_type in DEVICE_TYPES:
                    ...
                    for row in sorted_reader:
                        if device_type in row["Video Name"]:
                            if row["Mostly predicted label"] == "0":
                                ...
                                device11 += 1
                            elif row["Mostly predicted label"] == "11":
                                device12 += 1
                            elif row["Mostly predicted label"] == "12":
                                device13 += 1
                            elif row["Mostly predicted label"] == "13":
                                device14 += 1
                            elif row["Mostly predicted label"] == "14":
                                device15 += 1
                    device_array_total = [device1, device2, device3, device4, device5, device6, 
                                    device7, device8, device9, device10, device11, device12, device13, device14, device15]
                            ...
                            if device_type in row2["Video Name"] and video_type in row2["Video Name"]:
                                if video_type == "flat":
                                    if row2["Mostly predicted label"] == "0":
                                        ...
                                        device11_flat += 1
                                    elif row2["Mostly predicted label"] == "11":
                                        device12_flat += 1
                                    elif row2["Mostly predicted label"] == "12":
                                        device13_flat += 1
                                    elif row2["Mostly predicted label"] == "13":
                                        device14_flat += 1
                                    elif row2["Mostly predicted label"] == "14":
                                        device15_flat += 1
                    ...
                    device_array_flat = [device1_flat, device2_flat, device3_flat,
                                    ...
                                    device12_flat, device13_flat, device14_flat, device15_flat]
                    ...
                    device_array_indoor = [device1_indoor, device2_indoor, device3_indoor,
                                    ...
                                    device12_indoor, device13_indoor, device14_indoor, device15_indoor]
                    ...
                    device_array_outdoor = [device1_outdoor, device2_outdoor, device3_outdoor, 
                                    ...
                                    device12_outdoor, device13_outdoor, device14_outdoor, device15_outdoor]
                    main_stats_array_outdoor.append(device_array_outdoor)

            df_cm_total = pd.DataFrame(main_stats_array_total, range(15), range(15))
            df_cm_flat = pd.DataFrame(main_stats_array_flat, range(15), range(15))
            df_cm_indoor = pd.DataFrame(main_stats_array_indoor, range(15), range(15))
            df_cm_outdoor = pd.DataFrame(main_stats_array_outdoor, range(15), range(15))
