...
from model_files.constrained_net_patches import ConstrainedNetTransferPatches
...
parser.add_argument('--height', type=int, required=False, default=128, help='Input Height [default: 128]')
parser.add_argument('--width', type=int, required=False, default=128, help='Width of CNN input dimension [default: 128]')
...
def run_locally():
    ...
    return fc_size, fc_layers, n_epochs, cnn_height, cnn_width, batch_size, use_constrained_layer, model_path, model_name, dataset_path, dataset_path_noiseprints

if __name__ == "__main__":
    DEBUG = True

    if DEBUG:
        fc_size, fc_layers, n_epochs, cnn_height, cnn_width, batch_size, use_constrained_layer, model_path, model_name, dataset_path, dataset_path_noiseprints = run_locally()
    else:
    ...
    data_factory = DataSetGeneratorPatches(input_dir_frames=dataset_path,
                            input_dir_noiseprint=dataset_path_noiseprints)
    ...
    constr_net = ConstrainedNetTransferPatches(constrained_net=use_constrained_layer)
    if model_path:
