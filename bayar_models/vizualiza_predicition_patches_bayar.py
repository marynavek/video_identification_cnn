...
class PatchPredictionVisBayar:
    ...
    def start(self, input_file):
        ...
        img_scenario.savefig(os.path.join(self.result_dir, f"{self.model_name}_scenario_results.png"))
        img_platform = self.__plot_platform(df_frame_pred)
        img_platform.savefig(os.path.join(self.result_dir, f"{self.model_name}_platform_results.png"))
    ...
    def __plot_platform(self, df):
        ...
        plt.title(f"Patch Platform Accuracy")
        plt.xlim(1, xmax+1)
        plt.ylim(0, 1)
    ...
    def __plot_scenario(self, df):
        ...
        plt.title(f"{self.model_name} - Patch Scenario Accuracy ({num_epochs} epochs)")
        plt.xlim(0, xmax)
        plt.ylim(0, 1)
        plt.xticks(x_range)
