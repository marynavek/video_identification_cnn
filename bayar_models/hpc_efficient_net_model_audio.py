...
from tensorflow.keras.layers import Dense, Reshape, Input, GlobalAveragePooling2D, BatchNormalization, Dropout
...
class EfficientNetAudio:
    ...
    def __init__(self, model_path, tensor_flow_path):
        self.model = None
    ...
    def __generate_model_path(self, model_path):
        ...
        new_path = os.path.join(model_path, "cortivo_audio")
        return new_path
    ...
    def __generate_tensor_path(self, tensor_flow_path):
        ...
        new_path = os.path.join(tensor_flow_path, "cortivo_audio")
        return new_path
    ...
    def create_model(self, num_classes, model_name=None):
        ...
        reshape_input = Reshape((224, 224, 3), input_shape=input_shape)(inputs)
        # x = img_augmentation(inputs)
        model = EfficientNetB0(include_top=False,input_tensor=reshape_input, weights='imagenet')
        ...
        model = Model(inputs, outputs, name="EfficientNet_Audio")
    ...
    def train(self, train_ds, train_labels, val_ds_test, test_labels, num_classes):
        if self.model is None:
            raise ValueError("Cannot start training! self.model is None!")

        callbacks = self.get_callbacks()
        
        self.model.fit(train_ds, train_labels,
                       epochs=100,
                       initial_epoch=13,
                       validation_data=(val_ds_test, test_labels),
                       callbacks=callbacks,
                       workers=12,
