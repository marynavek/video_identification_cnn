...
class ConstrainedNet:
    GLOBAL_SAVE_MODEL_DIR = "/home/marynavek/Video_Project/files/models/"
    GLOBAL_TENSORBOARD_DIR = "/home/marynavek/Video_Project/files/tensorboard/"
    ...
    def set_tranfer_model(self, model_path):
        # Path is e.g. ~/constrained_net/fm-e00001.h5
        path_splits = model_path.split(os.sep)
        model_name = path_splits[-2]
        print(model_name)
        self.transfer_model = model_path
        self.transfer_model = model_name

        # if self.constrained_net:
        self.transfer_model = tf.keras.models.load_model(model_path, custom_objects={
                'Constrained3DKernelMinimal': Constrained3DKernelMinimal})
        # else:
    ...
    def train(self, train_ds, val_ds, epochs=1, initial_epoch=1):
        if self.model is None:
    ...
    def train_transfer_model(self, train_ds, val_ds, epochs=1, initial_epoch=0):
        if self.transfer_model is None:
            raise ValueError("Cannot start training! self.model is None!")
	...
	# evaluate the model
        if self.model:
            ...
            _, test_acc = self.transfer_model.evaluate(val_ds, verbose=0)
    ...
    def save_trained_model(self, model_name):
        if self.model:
            ...
            self.transfer_model.save(model_name)

    def load_trained_model(self, model_name):
        return tf.keras.models.load_model(model_name)
    
    def create_main_model_from_transfer(self, pre_trained_model_path, num_outputs, fc_size, height=480, width=800, freeze_layers=None):
        input_shape = (height, width, 3)
        ...
        for layer in base_model.layers:
            ...
            if freeze_layers:
                ...
                if "_3" not in layer.name and "_2" not in layer.name:
                    layer.trainable = False
