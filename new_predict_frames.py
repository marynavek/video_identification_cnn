...
class PredictFrames():
    ...
    def __init__(self, model_dir=None, results_dir=None, model_fname=None, constrained=False):
        ...
        #Loading the model
        model_path = os.path.joinh(model_dir, model_fname)
        if constrained:
    ...
    def __predict_and_save(self, test_dictionary, output_file):
        predict_ds_generator = DataGenerator(test_dictionary, shuffle=False)
        ...
        cce = tf.keras.losses.CategoticalCrossentropy(reduction=tf.keras.losses.Reduction.NONE)
        ...
        data_results = pd.DataFrames(list(zip(frames_list, actual_labels, predicted_labels, prediction_losses)), columns=["File", "True Label", "Predicted Label", "Loss"])
    ...
    def __get_files_names_and_labels(self, test_dictionary):
        ...
        actual_encoded_list = [val[label_key] for key, val in test_dictionary.items() if label_key in val]
        frames_list =  [val[frame_path] for key, val in test_dictionary.items() if frame_path in val]
