...
class DataFactory:
    ...
    def __init__(self, width=800, height=480, batch_size=32, input_dir=None, test_dir_suffix=""):
        ...
        print("Train and Test image count")
        print(self.train_image_count)
        print(self.test_image_count)
        print("batch Size")
...
#   labels = np.reshape(labels, (8, 1))
    ...
    def get_np_array_label_images(ds):
        labels_ds = ds.flat_map(lambda x, y: tf.data.Dataset.from_tensor_slices(y))
        the_labels = np.array(list(labels_ds.as_numpy_iterator())).astype(np.int32)
        data_ds = ds.flat_map(lambda x, y: tf.data.Dataset.from_tensor_slices(x))
        the_data = np.array(list(data_ds.as_numpy_iterator())).astype(np.int32)
