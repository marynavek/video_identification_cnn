    ...
    for video in original_videos:
        ...
        for video_variation in video_variations:
            ...
            if os.path.exists(vid_path):
                ...
                # # print(vid_path)
                # for frame in frames:
                #     image = cv2.imread(os.path.join(vid_path,frame))
                #     frame_prnu = extract_single(image)
                #     std = np.std(frame_prnu)
                #     addItem = {"frame": frame, "std": std}
                #     std_prnu.append(addItem)

                # sorted_frames_prnu = sorted(std_prnu, key=lambda x: x["std"])
                # if len(frames) > 5:
                #     for i in sorted_frames_prnu:
                #         if len(sorted_frames) >= 5:
                #             break
                #         else:
                #             sorted_frames.append(i["frame"])
                # else:
                ...
                # print(train_frames_1)
                for frame in sorted_frames:
                    frame_src_path = os.path.join(vid_path, frame)
                    image = cv2.imread(frame_src_path)
                    file_name = "frame_number_"+str(train_frames_1)+ "_vid_name_" + new_video_name + ".jpg"
                    cv2.imwrite(os.path.join(dest_path_1, file_name), image)
                    train_frames_1 += 1
                    
                    


input_frames_dir = "/Users/marynavek/Projects/files/15_devices_std_frames"
output_frames_dir_1 = "/Users/marynavek/Projects/files/new_split_tf_15_devices_std_frames"
...
val_frames_dir_1 = os.path.join(output_frames_dir_1, "val")
...
if not os.path.exists(val_frames_dir_1):
    os.mkdir(val_frames_dir_1)
...
for device in devices:
    # if device in DATASET_DEVICES:
        ...
        d_dest_val_path_1 = os.path.join(val_frames_dir_1, device)
        ...
        if not os.path.exists(d_dest_val_path_1):
            os.mkdir(d_dest_val_path_1)
        ...
        num_flat_test_vids= int(len(flat_vids)*0.3)
        num_flat_val_vids = int((len(flat_vids) - num_flat_test_vids)*0.3)
        if num_flat_val_vids == 0:
            num_flat_val_vids = 1
        num_flat_train_vids = len(flat_vids) - num_flat_test_vids - num_flat_val_vids

        num_indoor_test_vids= int(len(indoor_vids)*0.3)
        num_indoor_val_vids = int((len(indoor_vids) - num_indoor_test_vids)*0.3)
        if num_indoor_val_vids == 0:
            num_indoor_val_vids = 1
        num_indoor_train_vids = len(indoor_vids) - num_indoor_test_vids - num_indoor_val_vids

        num_outdoor_test_vids= int(len(outdoor_vids)*0.3)
        num_outdoor_val_vids = int((len(outdoor_vids) - num_outdoor_test_vids)*0.3)
        if num_outdoor_val_vids == 0:
            num_outdoor_val_vids = 1
        num_outdoor_train_vids = len(outdoor_vids) - num_outdoor_test_vids - num_outdoor_val_vids

        # if num_outdoor_val_vids == 0:
        #     num_outdoor_val_vids = 1

        # print(num_indoor_val_vids)
        # print(num_outdoor_val_vids)
        # print(num_flat_val_vids)

        num_test_vids = num_flat_test_vids + num_indoor_test_vids + num_outdoor_test_vids
        num_val_vids = num_flat_val_vids + num_indoor_val_vids + num_outdoor_val_vids
        num_train_vids = num_flat_train_vids + num_indoor_train_vids + num_outdoor_train_vids

        print(f"\n{device} | Total videos: {num_original_vids}, train: {num_train_vids}, test: {num_test_vids}, validation: {num_val_vids}\n")
        ...
        val_vids.extend(flat_vids[2:3])

        del flat_vids[0:3]  # Remove the used flat vids
        
        train_vids.extend(indoor_vids[0:1])
        test_vids.extend(indoor_vids[1:2])
        val_vids.extend(indoor_vids[2:3])

        del indoor_vids[0:3]

        train_vids.extend(outdoor_vids[0:1])
        test_vids.extend(outdoor_vids[1:2])
        val_vids.extend(outdoor_vids[2:3])

        del outdoor_vids[0:3]
        ...
        num_remaining_val_flat = num_flat_val_vids - int(len(val_vids)/3)
        num_remaining_val_indoor = num_indoor_val_vids - int(len(val_vids)/3)
        num_remaining_val_outdoor = num_outdoor_val_vids - int(len(val_vids)/3)

        # print(len(train_vids))
        # print(len(test_vids))
        # print(len(val_vids))

        # print(num_remaining_train_flat)
        # print(num_remaining_train_indoor)
        # print(num_remaining_train_outdoor)

        # print(num_remaining_test_flat)
        # print(num_remaining_test_indoor)
        # print(num_remaining_test_outdoor)

        # print(num_remaining_val_flat)
        # print(num_remaining_val_indoor)
        # print(num_remaining_val_outdoor)

        if num_remaining_train_indoor < 0:
            num_remaining_train_indoor = 0
        if num_remaining_test_indoor < 0:
            num_remaining_test_indoor = 0
        if num_remaining_val_indoor < 0:
            num_remaining_val_indoor = 0
        
        print(num_remaining_train_indoor)
        print(num_remaining_test_indoor)
        print(num_remaining_val_indoor)

        print(len(unused_indoor))
        if len(unused_indoor) == 0:
            num_remaining_train_indoor = 0
        # print(num_remaining_test_indoor)
        # print(num_remaining_val_indoor)
        ...
        ##
        for i in range(num_remaining_val_flat):
            val_vids.append(unused_flat[i])
        
        del unused_flat[0:num_remaining_val_flat]

        for i in range(num_remaining_val_indoor):
            val_vids.append(unused_indoor[i])
        
        del unused_indoor[0:num_remaining_val_indoor]

        for i in range(num_remaining_val_outdoor):
            val_vids.append(unused_outdoor[i])
        
        del unused_outdoor[0:num_remaining_val_outdoor]
        ...
        combined_2 = set(val_vids).intersection(test_vids)
        if len(combined_2) > 0:
            print("Error! The following video(s) occur in both test and unused set:")
            for item in combined_2:
                print(f"{item}")
        ...
        copy_frames(d_src_path, d_dest_val_path_1, val_vids, device)
        ...
        print(f"{device} | Val ({len(val_vids)}): {val_vids}\n")