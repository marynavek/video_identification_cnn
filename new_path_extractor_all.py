...
def get_patches(img_data, std_threshold, max_num_patches):
    ...
    noise = extract_single(img_data)
    default_patch_std = np.std(noise)
    ...
    for row_idx in range(patch.height, image.height, stride.height_step):
        for col_idx in range(patch.width, image.width, stride.width_step):
            ...
            # noise_patch = extract_single(cropped_img)
...
if __name__ == "__main__":
    images_per_device = Path('/Users/marynavek/Projects/files/ten_dev_train_test_frames/test/')

    patches_per_device_top_rigth = Path('/Users/marynavek/Projects/files/test_ten_homo_patches_1st_sector/test/')
    patches_per_device_bottom_right = Path('/Users/marynavek/Projects/files/test_ten_homo_patches_2nd_sector/test/')
    patches_per_device_top_left = Path('/Users/marynavek/Projects/files/test_ten_homo_patches_3rd_sector/test/')
    patches_per_device_bottom_left = Path('/Users/marynavek/Projects/files/test_ten_homo_patches_4th_sector/test/')
