...
if __name__ == "__main__":
    ...
    files_path = os.path.join(output_folder, "initial_stats_new")
    ...
    for i, file in enumerate(predicted_files):
        ...
        with open(path, newline='') as csvfile:
            ...
            for row in reader:
                ...
                frame_name_temp = row["File"].split("frame_")[1]
                frame_name = frame_name_temp.split("_vid_name")[0]
                # print(video_name)
                row["Video Name"] = video_name
                row["Frame Name"] = frame_name
                newreader.append(row)
                

            # print(newreader)
        file_name = 'new_output_frames_stats_' + str(i) + '_.csv'
        csv_path = os.path.join(files_path, file_name)
        with open(csv_path, 'w') as csvfile:
            fieldnames = reader.fieldnames
            fieldnames.append("Video Name")
            fieldnames.append("Frame Name")
