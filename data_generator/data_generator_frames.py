...
class DataGeneratorFrames(Sequence):
    ...
    def __getitem__(self, index):
        ...
        # Generate data
        frames_batch, noise_batch, labels_batch = self.__generate_frames_ds__(list_IDs_temp)

        if self.to_fit == True:
            return [noise_batch, frames_batch], labels_batch
        else:
            with open('true_labels_for_prediction.csv', 'a', newline='') as f_object:  
                ...
                f_object.close()
            return [noise_batch, frames_batch]
    ...
    def __generate_frames_ds__(self, list_IDs_temp):
        ...
        noise_ds = np.empty((self.batch_size, 480, 800, 3), dtype=np.uint8)
        ...
        for i, id in enumerate(list_IDs_temp):
            ...
            noise = self.__get_image__(item["noise_path"])
